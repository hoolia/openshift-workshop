...
In this lab you can use a build webhook to trigger a build execution every time there is a change in the nationalparks BitBucket repository. In the OpenShift web console, navigate to your `{{ username }}` project, and then mouse-over `Builds` and then `Builds`. Click the `nationalparks` build and then `Configuration` tab.
...
that you have on your local BitBucket:
...
Click "Project" at the top of the BitBucket page, and then "Files" towards the
middle of the page. This is BitBucket's repository view. 
